...
def test_requester_types_persistence(temp_db):
    ...
    teacher = Requester(name="Prof. Smith", requester_type="teacher", department="Science")
    assert teacher.save() is True
    
    # Student with grade/section
    student = Requester(name="Student 1", requester_type="student", grade_level="10", section="B")
    assert student.save() is True

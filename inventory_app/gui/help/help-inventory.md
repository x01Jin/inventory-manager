...
- **Stocks parsing:** The importer accepts a variety of free-form `stocks` values. Size-bearing entries like `900ml`, `1.1 L`, `2 liters`, `125 g` are treated as **usable quantities** using the numeric part (for example, `900ml` -> quantity `900`) and the parsed size is used as the item `size` when an explicit `size` column is empty. Invalid or unparseable stock values cause the row to be **skipped** and the reason is recorded in the import log.

...
- **Minimum required columns:** `name` (or `item` / `items`), `item type`, and either `stocks` or the Stock Levels Report headers `original stock` + `current stock`. If these are missing the import aborts with an error telling you which columns are missing.
...
- **Stock Levels Report imports:** If the sheet includes `original stock` and `current stock` columns, the importer uses `original stock` for the initial batch and applies a stock adjustment so the computed current stock matches the provided value.
...
- **🔍 Search:** Search by item name, category, supplier, or PO number. Typing updates the table instantly.
...
- **PO Number:** Optional purchase order reference for procurement tracking and report export.
...
- The **Search** box matches text inside `name`, `category`, `supplier`, or `po_number` (case-insensitive, substring match).

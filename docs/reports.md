...
- Implemented inventory report types (UI and code):
  ...
  - Update History Report — history of edits to inventory items with editor name, timestamp, reason, and item PO Number. Addresses beta test requirement #7.
    - Batch synchronization entries include explicit create/remove events plus field-level old/new deltas for batch number, received date, received quantity, and disposal date.
  ...
  - Audit Log Report — centralized audit stream across item/requisition updates, disposals, defective recordings, and activity events.
    ...
    - Requester add/edit/delete events carry `entity_id` links to the requester row and include editor attribution.
    - Automatic requisition status transitions generated by refresh/date watchers are logged as requisition history events with `System Auto` attribution.
    ...
    - Audit export rows are normalized to avoid empty cells. When source data is blank, placeholder values are emitted (for example `N/A`, `System`, or `Unknown timestamp`).
    - `Change Details` is always populated. Events without field-level deltas use `No field-level change details recorded`.
